...
module.exports = function initialize (adapter, opts) {
  ...
  if (process.env.DB_MAX_RESULTS) {
    try {
      let newMaxResults = parseInt(process.env.DB_MAX_RESULTS)

      if (Number.isNaN(newMaxResults) || !Number.isInteger(newMaxResults) || newMaxResults <= 0) {
        throw new Error('LEVELDB_MAX_RESULTS must be a positive non-zero integer')
      } else {
        maxResults = newMaxResults
      }
    } catch(e) {
      debug('Invalid value for LEVELDB_MAX_RESULTS', e)
    }
  }
  ...
  router.get(apiPath + '/status', (req, res) => {
    parallel({
      ...
      let localheight = results.localtip.script ? results.localtip.script.height : null
      let bitcoinheight = results.bitcoinheight
      let heightBehind = bitcoinheight && localheight ? bitcoinheight - localheight : null
      status = {
        chainBlock: bitcoinheight,
        indexBlock: localheight,
        network: networkName,
        blocksBehind: heightBehind,
        ready: heightBehind <= 0
      }
  ...
  function addressToScriptId(address) {
    ...
    debug('Output script id: ', script.toString('hex'))
  ...
  router.get(apiPath + '/a/:address/utxos', (req, res) => {
    ...
    parallel({
      ...
      let tipHeight = results.tip.script.height
      let utxos = []
  ...
  router.get(apiPath + '/a/:address/txs', (req, res) => {
    ...
    searchRange('transactionIdsByScriptRange', scId, fromHeight, toHeight, mem, (err, txIdSet) => {
      ...
      parallel({
        ...
        let txs = []
        let tipHeight = tip.script.height
