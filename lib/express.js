...
module.exports = function initialize (adapter, opts) {
  ...
  let searchRange = (fn, scId, from, to, mem, cb) => adapter[fn]({scId, heightRange: [from, to], mempool: mem}, 3000000, cb)
  ...
  router.get(apiPath + '/a/:address/utxos', (req, res) => {
    ...
    let fromHeight = parseInt(req.query.from)
    let toHeight = parseInt(req.query.to)
    if (Number.isNaN(fromHeight) || fromHeight < 0) {
      fromHeight = 0
    }
    if (Number.isNaN(toHeight) || toHeight < 0) {
      toHeight = 0xffffffff
    }
    let mem = req.query.nomem ? false : true
  ...
  router.get(apiPath + '/a/:address/txs', (req, res) => {
    ...
    let mem = req.query.nomem ? false : true
    let verbose = parseInt(req.query.verbose)
    if (Number.isNaN(verbose)) verbose = 0 
